...
class RewstApp {
  ...
  async _fetchChunkAdaptiveRetry(startDay, endDay, chunkSizeIndex, workflowId, orgIds, allResults = []) {
    ...
    while (currentEnd > startDay) {
      ...
      try {
        ...
        const chunkExecutions = await this._fetchExecutionsChunkSingle(currentStart, currentEnd, workflowId, orgIds, { timeout: timeoutMs });
        const elapsed = Date.now() - fetchStart;
   ...
   * @private
   */
  async _fetchChunkAdaptiveRetryLightweight(startDay, endDay, chunkSizeIndex, workflowId, orgIds, allResults = []) {
    ...
    while (currentEnd > startDay) {
      ...
      try {
        ...
        const lightweightExecutions = await this._fetchExecutionsChunkSingleLightweight(
          currentStart, currentEnd, workflowId, orgIds, { timeout: timeoutMs }
        );
  ...
  const processOrg = async (orgId, orgIndex) => {
    ...
    try {
      ...
      const orgExecs = await this._fetchChunkAdaptiveSingleOrg(
        ...
        { timeout: 45000, deadline, ...options }  // 45s per org, pass deadline + scope
      );
  ...
  const retryOrgTask = async (orgId) => {
    ...
    try {
      ...
      const orgResults = useLightweight
        ? await this._fetchChunkAdaptiveRetryLightweight(range.start, range.end, 0, workflowId, [orgId], [])
        : await this._fetchChunkAdaptiveRetry(range.start, range.end, 0, workflowId, [orgId], []);
