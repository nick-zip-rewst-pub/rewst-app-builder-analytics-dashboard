...
class RewstApp {
  ...
  static CHUNK_TIMEOUTS = {
    6: 5000,     // 5 seconds for 6-day chunks
    3: 5000,     // 5 seconds for 3-day chunks
    2: 5000,     // 5 seconds for 2-day chunks
    1: 5000,     // 5 seconds for 1-day chunks
...
async _fetchExecutionsMultiOrg(daysAgoStart, daysAgoEnd, workflowId, orgIds, options = {}) {
  ...
  const earlyReturnThreshold = Math.ceil(totalBatches * 0.8); // Return when 80% done
  const maxWaitMs = 15000; // Max 15s before returning with what we have

  this._log(`Fetching ${orgIds.length} orgs in ${totalBatches} batches (return early at ${earlyReturnThreshold}/${totalBatches} or ${maxWaitMs/1000}s)`);
      ...
      .then(() => {
        ...
        return this._fetchExecutionsChunkSingle(daysAgoStart, daysAgoEnd, workflowId, batchOrgIds, options)
          .then(results => {
            ...
            if (completedResults.length >= earlyReturnThreshold && resolveEarly) {
              resolveEarly();
              resolveEarly = null;
            }
          ...
          .catch(error => {
            ...
            // Still count toward early return
            if (completedResults.length >= earlyReturnThreshold && resolveEarly) {
              resolveEarly();
              resolveEarly = null;
            }
